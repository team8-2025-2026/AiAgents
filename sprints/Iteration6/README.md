...
Хотим реализовать чат Student-(Assistent/LLM), с возможностью смены собеседника (с одной стороны) а так же если чат происходит с LLM-кой, то все сообщения пользователя переадресуются в LLM API, а ответы LLM-ки возвращаются как сообщения.
