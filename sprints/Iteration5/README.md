...
2. Создание WebUI, ожидаемый функционал:
	...
	2. `/auth` - страница авторизации, для демо пока будем использовать страндартные Имя+Пароль. Создание аккаунта перекладывается на руки поддержки (не будет функционала создания аккаунта)
	3. `/user` - страница информации пользователя. Имя, Фамилия, статус (Студент/Поддержка/Учитель), описание.
...
Создание пользователя. `access_token` - токен авторизации пользователя. Данный метод должен быть доступен только для ассистентов.
...
Обновление данных пользователя. `access_token` - токен авторизации пользователя. Данный метод должен быть доступен только для ассистентов
